...
namespace Nfro.Core.Providers {
    public class TokenProvider : DataProvider{
        ...
        public void SaveToken(Token token) {
            ...
            TokenRow savedTokenRow = db.Single<TokenRow>(@"WHERE UserId = @0 AND DeviceId = @1", token.UserId, (int)token.DeviceType);
        ...
        public Boolean isValid(string token, int userId) {
            TokenRow savedTokenRow = db.Single<TokenRow>(@"WHERE UserId = @0", userId);
            if(savedTokenRow == null) {
                return false;
            }
