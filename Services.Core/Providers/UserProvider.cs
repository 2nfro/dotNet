...
namespace Nfro.Core.Providers {
    public class UserProvider : DataProvider {
        ...
        public long GetUserId(String email, String pw) {
            UserRow user = db.SingleOrDefault<UserRow>("WHERE Email=@1", TABLE, email);
        ...
        public bool ContainsEmail(String email) {
            if(db.SingleOrDefault<UserRow>("WHERE Email=@0", email) != null) {
                return true;
            }
            return false;
        }
        ...
        public bool IsUserActive(long userId) {
            UserRow userRow = db.SingleOrDefault<UserRow>(userId);
            if(userRow == null) {
                return false;
            }
        ...
        public void ActivateUser(long id) {
            UserRow user = db.SingleOrDefault<UserRow>(id);
            if(user != null) {
                user.Active = true;
                db.Update(user);
            }
        ...
        public UserInfo GetUserInfo(int userId) {
            UserRow user = db.SingleOrDefault<UserRow>(userId);
            if(user == null) {
                return null;
            }
